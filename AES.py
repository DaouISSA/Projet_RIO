...
def encrypt(plaintext, key, rounds):
    ...
    # Initial AddRoundKey
    state = add_round_key(state, round_keys[0].astype(np.uint8))

    # Rounds 1..(rounds-1)
    for r in range(1, rounds):
        state = np.array(sub_bytes(state), dtype=np.uint8)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, round_keys[r].astype(np.uint8))

    # Final round (no MixColumns)
    state = np.array(sub_bytes(state), dtype=np.uint8)
    state = shift_rows(state)
    state = add_round_key(state, round_keys[rounds].astype(np.uint8))
...
def decrypt(ciphertext, key, rounds):
    ...
    state = np.array(inv_sub_bytes(state), dtype=np.uint8)
    ...
    # Final AddRoundKey with round 0
    state = add_round_key(state, round_keys[0].astype(np.uint8))
    """Votre code"""
...
key_mac = [0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0xcf, 0x80, 0x09, 0x89, 0x01, 0x0c]
key_enc = [0x64, 0xa7, 0x08, 0x23, 0xfe, 0xea, 0x86, 0xe1, 0xba, 0x7f, 0x09, 0x56, 0x1a, 0xbb, 0x00, 0xca]
iv = [0x04] * 16  # Exemple d'IV de 16 octets
nonce = [0x03] * 8  # Exemple de nonce de 8 octets


""" Message initial """
plaintext = [0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d, 0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34, 0x21, 0x2a, 0x11, 0x9e, 0x1a, 0x00, 0x81, 0x50, 0xb1, 0xe5, 0xee, 0x77, 0x12, 0xe7, 0x70, 0x28]
